	...
	function woo_custom_typography() {
		...
		if ( $woo_options[ 'woo_typography' ] == "true") {
			...
			if ( $woo_options[ 'woo_font_widget_titles' ] )
				$output .= '.widget h3, .widget.widget_woo_subscribe .title { '.woo_generate_font_css($woo_options[ 'woo_font_widget_titles' ]).' }'  . "\n";	
		...
		// Add default typography Google Font
			...
			if ( !empty( $woo_options['woo_default_face'] ) )
				$output .= 'h1, h2, h3, h4, h5, h6, .widget h3, .post .title, .archive_header, .widget.widget_woo_subscribe .title { '.woo_generate_font_css($woo_options['woo_default_face']).' }' . "\n";			

<?php
/*
Template Name: Portfolio Four Wide
*/
get_header(); ?>
...
<li data-id="post-<?php the_ID(); ?>" data-type="<?php $categories = get_the_category(); $count = count($categories); $i=1; foreach($categories as $category) {	echo str_replace('-', '', $category->slug); if ($i<$count) echo ' '; $i++;} ?>" class="post-<?php the_ID(); ?> <?php $categories = get_the_category(); foreach($categories as $category) {	echo str_replace('-', '', $category->slug).' '; } ?> project">                    
	<div class="single_featured_wrap single_featured_wrap_fourth">
		<div class="fourth_wrap info_wrap">
			<?php if (is_sticky()) { ?><span class="sticky_star"></span><?php } ?>
			<a href="<?php the_permalink() ?>" class="touch_link"></a>
			<div class="hover_info">
				<div class="hover_content">
					<h4><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h4>
					<div class="port_cat_position"><?php the_category(', ') ?> / <?php the_time('F d, Y'); ?></div>
				...
				<a href="<?php the_permalink() ?>" class="view_post"><?php _e('View Post', 'designcrumbs'); ?></a>
				</div>
			</div>
			<div class="fourth_shadow"></div>
			<?php the_post_thumbnail( 'port_image4', array('alt' => get_the_title(), 'title' => get_the_title()) ); ?>
		</div>
	</div>
</li>
<?php } ?>
